...
xdel(winsid()) // cierra ventanas gracas

// Definicion de la funcion
function y = f(x)
y = x.*x;
endfunction

// Calculo de la derivada utilizando diferencias finitas
...
df_approx = dfa(f,x,h); // Evaluacion de la derivada por diferencias finitas
df_scilab = derivative(f,x,order=1); // Derivada obtenida por derivative
...
// Grafica
plot(ih,log10(err_rel),'b*-'); // Grafica en escala logaritmica en el eje y
xtitle('Error relativo utilizando diferencias finitas');
xlabel('i');
